...
const Services = () => {
  ...
  return (
    <section
      id="services"
      className="bg-[#0F172A] p-4 sm:p-8 flex flex-col items-center justify-center py-12"
    >
      <h2 className="text-3xl sm:text-4xl text-white  mb-2">Services</h2>
      <p className="text-gray-500 text-sm  mb-8">What I Offer</p>
       
      <div className="grid gap-6 sm:gap-8 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-3 max-w-6xl mx-auto mt-20 mb-20">
        {services.map((work) => (
          <article
            key={work.id}
            className="p-6 sm:p-8 flex flex-col items-start justify-end gap-4 bg-[#0f1625]
             text-white h-72 sm:h-80 md:h-96 w-full rounded-lg shadow-lg
